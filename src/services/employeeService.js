  ...
  try {
    ...
    if (employee.contact) {
      const { data: contactData, error: contactError } = await supabase
        .from('contact_info')
        .insert([{ phone: employee.contact.phone, email: employee.contact.email }])
        .select()
        .single();
    ...
    if (employee.address) {
      const { data: addressData, error: addressError } = await supabase
        .from('address')
        .insert([{ street: employee.address.street, city: employee.address.city, state: employee.address.state, postal_code: employee.address.postal_code }])
        .select()
        .single();
    ...
    const { data, error } = await supabase
      .from('employee')
      .insert([{ first_name: employee.first_name, last_name: employee.last_name, document_type: employee.document_type, document_number: employee.document_number, position: employee.position, department: employee.department, employee_code: employee.employee_code, contact_id: contactId, address_id: addressId }])
  ...
  try {
    if (updatedData.contact && updatedData.contact_id) {
      const { error: cErr } = await supabase
        .from('contact_info')
        .update({ phone: updatedData.contact.phone, email: updatedData.contact.email })
    ...
    if (updatedData.address && updatedData.address_id) {
      const { error: aErr } = await supabase
        .from('address')
        .update({ street: updatedData.address.street, city: updatedData.address.city, state: updatedData.address.state, postal_code: updatedData.address.postal_code })
        .eq('id', updatedData.address_id);
      if (aErr) throw aErr;
    }
    const { data, error } = await supabase
      .from('employee')
      .update({ first_name: updatedData.first_name, last_name: updatedData.last_name, document_type: updatedData.document_type, document_number: updatedData.document_number, position: updatedData.position, department: updatedData.department, employee_code: updatedData.employee_code })
      .eq('id', id)
      .select(`*, contact_info:contact_id(*), address:address_id(*)`)
      .single();
    if (error) throw error;
...
export async function createAttendanceRecord(record) {
  try {
    const { data, error } = await supabase
      .from('attendance_record')
      .insert([{ employee_id: record.employee_id, date: record.date, check_in: record.check_in, check_out: record.check_out, source_terminal: record.source_terminal, status: record.status ?? true }])
      .select()
      .single();
    if (error) throw error;
    return data;
